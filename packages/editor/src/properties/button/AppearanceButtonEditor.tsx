...
export const AppearanceButtonEditor = ({ field, onChange }: AppearanceButtonEditorProps) => {
  ...
  return (
    <SectionPanel title="Appearance">
      <FormItem>
        <Label className="tw-block tw-text-sm tw-font-medium">Button color</Label>
        <div className="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
          <Button color="primary" size="sm" onClick={() => handleColorChange('primary')}>
            Primary
            {(field as ButtonField).appearance?.color === 'primary' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
          <Button color="secondary" size="sm" onClick={() => handleColorChange('secondary')}>
            Secondary
            {(field as ButtonField).appearance?.color === 'secondary' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
        </div>
      </FormItem>
      <FormItem className="!mt-5">
        <Label className="tw-block tw-text-sm tw-font-medium">Button variant</Label>
        <div className="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
          <Button color="primary" size="sm" variant="fill" onClick={() => handleVariantChange('fill')}>
            Fill
            {(field as ButtonField).appearance?.variant === 'fill' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
          <Button color="primary" size="sm" variant="outline" onClick={() => handleVariantChange('outline')}>
            Outline
            {(field as ButtonField).appearance?.variant === 'outline' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
          <Button color="primary" size="sm" variant="ghost" onClick={() => handleVariantChange('ghost')}>
            Ghost
            {(field as ButtonField).appearance?.variant === 'ghost' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
          <Button color="primary" size="sm" variant="shadow" onClick={() => handleVariantChange('shadow')}>
            Shadow
            {(field as ButtonField).appearance?.variant === 'shadow' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
        </div>
      </FormItem>
      <FormItem className="!mt-5">
        <Label className="tw-block tw-text-sm tw-font-medium">Button size</Label>
        <div className="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
          <Button color="secondary" size="xs" onClick={() => handleSizeChange('xs')}>
            XS
            {(field as ButtonField).appearance?.size === 'xs' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
          <Button color="secondary" size="sm" onClick={() => handleSizeChange('sm')}>
            SM
            {(field as ButtonField).appearance?.size === 'sm' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
          <Button color="secondary" size="default" onClick={() => handleSizeChange('default')}>
            Default
            {(field as ButtonField).appearance?.size === 'default' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
          <Button color="secondary" size="lg" onClick={() => handleSizeChange('lg')}>
            LG
            {(field as ButtonField).appearance?.size === 'lg' && <CheckCircleIcon className="tw-size-4" />}
          </Button>
        </div>
      </FormItem>
      <FormItem orientation="horizontal" className="!tw-mt-5">
        <div className="tw-col-span-4 tw-space-y-1.5">
          <Label htmlFor="sticky-at-bottom" className="tw-block tw-text-sm tw-font-medium">

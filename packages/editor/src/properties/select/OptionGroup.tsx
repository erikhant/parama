...
export const OptionGroup = memo<OptionGroupProps>(
  ({ field, group, groupIndex, onUpdate, onDelete, onAddItem }) => {
    const handleGroupLabelChange = useCallback(
      (e: React.ChangeEvent<HTMLInputElement>) => {
        onUpdate(groupIndex, { label: e.target.value }, field.defaultValue);
      },
      [groupIndex, onUpdate]
    );
    ...
    const handleItemUpdate = useCallback(
      (itemId: string, key: string, value: string) => {
        ...
        onUpdate(groupIndex, { items: newItems }, field.defaultValue);
      },
    ...
    const handleItemDelete = useCallback(
      (itemId: string) => {
        ...
        if (deletedItem && field.defaultValue === deletedItem.value) {
              ...
              <>
                ...
                <div className="flex items-center ml-4 gap-2 mt-2">
                  ...
                  <Switch
                    ...
                    checked={field.defaultValue === item.value}
                    onCheckedChange={(checked) => {
                      if (checked) {
                        handleDefaultValueChange(group, item.value);
                      } else if (field.defaultValue === item.value) {
